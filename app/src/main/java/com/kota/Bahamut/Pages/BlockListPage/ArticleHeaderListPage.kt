...
import com.kota.Bahamut.Service.UserSettings.setBlockList
...
class ArticleHeaderListPage : TelnetPage()() implements BlockListClickListener {
    var _inputField: EditText

    var articleHeaderListAdapter: BlockListAdapter
    // 按下新增
    private val _addListener = View.OnClickListener { v ->
        if var !: (_inputField = null) {
            ...
            if (block_name.length() > 0) {
                var new_list: List<String> = ArrayList<>(Arrays.asList(UserSettings.getArticleHeaders()));
    ...
    private var _articleHeadersList: List<String> = ArrayList<>();
    ...
    private fun showNotification(): Unit {
        var show_top_bottom_function: Boolean = NotificationSettings.getShowHeader();
    ...
    private fun reload(): Unit {
        var _temp: List<String> = Arrays.asList(UserSettings.getArticleHeaders());
    ...
    onBlockListPage_ItemView_delete_clicked(BlockListViewHolder blockListPage_ItemView): Unit {
        ...
        var (deleted_index: if ==0) {
            ASToast.showShortToast(getContextString(R.String.article_header_zero_error_message));
            return;
        }
        var new_list: List<String> = ArticleHeaderListPage._articleHeadersList;
