...
class BahamutCommandTheSameTitleDown(private var _article_index: Int) : TelnetCommand() {
    
    init {
        Action = BahamutCommandDefs.TheSameTitleDown
    }

    override fun execute(aListPage: TelnetListPage) {
        if (aListPage.listType > 0) {
            if (_article_index == aListPage.itemSize) {
                ASRunner {
