...
public class BoardLinkPage extends BoardPage {
  public String getListIdFromListName(String paramString) {
    return paramString + "[Board][TitleLinked]";
  }
    ...
    ASAlertDialog.createDialog().setTitle("加入書籤").setMessage("是否要將此標題加入書籤?").addButton("取消").addButton("加入").setListener(new ASAlertDialogListener() {
          ...
          public void onAlertDialogDismissWithButtonIndex(ASAlertDialog param1ASAlertDialog, int param1Int) {
            if (param1Int == 1) {
              ...
              param1ASAlertDialog = null;
              if (BoardLinkPage.this.getCount() > 0)
                boardPageItem = (BoardPageItem)BoardLinkPage.this.getItem(0); 
              if (boardPageItem != null) {
                ...
                bookmark.setKeyword(boardPageItem.Title);
                bookmark.setTitle(bookmark.generateTitle());
                BookmarkStore bookmarkStore = new BookmarkStore(BoardLinkPage.this.getContext());
                bookmarkStore.getBookmarkList(BoardLinkPage.this.getListName()).addBookmark(bookmark);
                bookmarkStore.store();
              } 
            } 
          }
...
/* Location:              C:\Users\kodak\Downloads\反編譯\dex-tools-v2.4\classes-dex2jar.jar!\com\kumi\Bahamut\Pages\BoardLinkPage.class
